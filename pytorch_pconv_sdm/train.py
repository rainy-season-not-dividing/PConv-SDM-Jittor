...
from pytorch_pconv_sdm.tools.metrics import AverageMeter, MeanIoU, PDFA


def parse_args():
    parser = argparse.ArgumentParser(description="Train the PyTorch PConv-SDM baseline.")
    ...
    parser.add_argument("--save-dir", type=str, default=osp.join("runs", "pytorch_pconv_sdm"))
...
def train_one_epoch(model, loader, optimizer, criterion, device, warm_epoch, epoch):
    model.train()
    meter = AverageMeter()
...
def main():
    ...
    train_set = IRSTDDataset(args.dataset_dir, mode="train", base_size=args.base_size, crop_size=args.crop_size)
    val_set = IRSTDDataset(args.dataset_dir, mode="val", base_size=args.base_size, crop_size=args.crop_size)
    train_loader = DataLoader(train_set, args.batch_size, shuffle=True, drop_last=True, num_workers=args.num_workers)
    val_loader = DataLoader(val_set, 1, shuffle=False, drop_last=False, num_workers=args.num_workers)

    model = MSHNet(input_channels=3, use_pconv=ablation["use_pconv"]).to(device)
    criterion = build_loss(ablation["loss_type"])
    optimizer = Adagrad(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        train_loss = train_one_epoch(model, train_loader, optimizer, criterion, device, args.warm_epoch, epoch)
        metrics = evaluate(model, val_loader, device, args.base_size)
        line = (
        ...
        if metrics["IoU"] > best_iou:
            ...
            torch.save(model.state_dict(), osp.join(run_dir, "best_weight.pkl"))
